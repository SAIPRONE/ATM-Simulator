...
const transaction = {
  ...
  getTransactions: function(id,callback){
    return db.query('select transaction.transaction as "Transaction type", concat(fname," ",lname) as "Client name", balance as "Balance", amount as "Amount", date_format(transaction_date,"%d.%m.%Y") as "Date" from client inner join account on client.id_client=account.id_client inner join transaction on transaction.id_account=account.id_account inner join card on card.id_card=transaction.id_card where card.id_card=? ORDER BY transaction.transaction_date DESC;',[id],callback);
